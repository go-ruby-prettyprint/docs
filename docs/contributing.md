...
Contributions are welcome. `go-ruby-prettyprint/prettyprint` is built to a small set of
...
- **Differential testing against MRI.** Correctness is defined by reference Ruby. A corpus
  is run through both `ruby -rprettyprint` and this library and the output is compared
...
- **The engine, not the inspector.** This module is the layout engine only. Anything that
  walks an object graph and emits `text`/`group`/`breakable` calls belongs in the consumer
  (the `pp` inspector in rbgo), not here.
...
The library is in
[`github.com/go-ruby-prettyprint/prettyprint`](https://github.com/go-ruby-prettyprint/prettyprint).
This documentation site is in
[`github.com/go-ruby-prettyprint/docs`](https://github.com/go-ruby-prettyprint/docs). Start
from the [Usage & API](api.md) page and the [Roadmap](roadmap.md) to find the right place
for your change.
